...
```{r}
# Business
busi_confu_matrix <- busi_biletter[,1:2] %>% 
  table()  %>% tbl_df() %>%
...
```{r}
# Politics
politics_confu_matrix <- politics_biletter[,1:2] %>% 
  table()  %>% tbl_df() %>%
  mutate(X1 = factor(X1), X2 = factor(X2, levels = rev(unique(X2)))) 

politics_confu_matrix %>%
  ggplot(., aes(x=X1, y=X2, fill=n)) +
  geom_tile() + theme_bw() + coord_equal() +
  scale_fill_distiller(palette="Greens", direction=1) +
...
```{r}
# Entertainment
entertain_confu_matrix <- entertain_biletter[,1:2] %>% 
  table()  %>% tbl_df() %>%
  mutate(X1 = factor(X1), X2 = factor(X2, levels = rev(unique(X2)))) 

entertain_confu_matrix %>%
  ggplot(., aes(x=X1, y=X2, fill=n)) +
  geom_tile() + theme_bw() + coord_equal() +
  scale_fill_distiller(palette="Greens", direction=1) +
...
```{r}
# Sport
sport_confu_matrix <- sport_biletter[,1:2] %>% 
  table()  %>% tbl_df() %>%
  mutate(X1 = factor(X1), X2 = factor(X2, levels = rev(unique(X2)))) 
...
```{r}
# Technology
tech_confu_matrix <- tech_biletter[,1:2] %>% 
  table()  %>% tbl_df() %>%
  mutate(X1 = factor(X1), X2 = factor(X2, levels = rev(unique(X2)))) 
tech_confu_matrix %>%
  ggplot(., aes(x=X1, y=X2, fill=n)) +
