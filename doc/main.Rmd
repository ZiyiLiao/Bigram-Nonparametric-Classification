...
ind <- cbind(busi_ind, politics_ind, entertain_ind, sport_ind, tech_ind)
...
test_error <- sum(test_pred != as.factor(test_feat$labels))/length(test_feat$labels)
...
atheletics_pred <- predict(fit_svm, athletics_feat$features)
...
results <- rbind(table(atheletics_pred),table(cricket_pred), table(football_pred),table(rugby_pred),table(tennis_pred))
rownames(results) <- c("athletics", "cricket", "football", "rugby","tennis")
results
```
```{r}
err <- 1-apply(results, 2, sum)/sum(results)
cat("error:",err[4])
```

### Step 4: KLD
```{r}
file <- train_feat$features[1,]

busi_train <- list.files(busi_location) %>% 
  lapply(., read_txt,busi_location) %>% 
  dictionary() %>% toString() %>% density()
save(busi_train, file = "../output/busi_train.RData")

sport_train <- list.files(sport_location) %>%
  lapply(., read_txt,sport_location) %>% 
  dictionary() %>% toString() %>% density()
save(sport_train, file = "../output/sport_train.RData")

politics_train <- list.files(politics_location) %>% 
  lapply(., read_txt,politics_location) %>% dictionary() %>%
  toString() %>% density()
save(politics_train, file = "../output/politics_train.RData")


entertain_train <- list.files(entertain_location) %>% 
  lapply(., read_txt,entertain_location) %>% dictionary() %>%
  toString() %>% density()
save(entertain_train, file = "../output/entertain_train.RData")

tech_train <- list.files(tech_location) %>% 
